...
export function GigaAMModelManager({
  ...
  useEffect(() => {
    ...
    const initializeModels = async () => {
      try {
        ...
        const modelList = await GigaAMAPI.getAvailableModels();
        setModels(modelList);

        setInitialized(true);
      } catch (err) {
        console.error('Failed to initialize GigaAM:', err);
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      console.log('[GigaAMModelManager] Setting up event listeners...');
      ...
      unlistenProgress = await listen<{ modelName: string; progress: number }>(
        ...
        (event) => {
          ...
          if (shouldUpdate) {
            console.log(`[GigaAMModelManager] Progress update for ${modelName}: ${progress}%`);
    ...
    return () => {
      console.log('[GigaAMModelManager] Cleaning up event listeners...');
  ...
  const deleteModel = async (modelName: string) => {
    ...
    try {
      await GigaAMAPI.deleteCorruptedModel(modelName);
